...
export default function ManageTicketsTable({ initialTickets }) {
  ...
  // Client Side Pagination Pipelines state control
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 8;

  // Confirmation Modal Handling Object States
  ...
  const handleStatusUpdateProcess = async () => {
    ...
    try {
    //   const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/tickets/status/${ticketId}`, {
    //     method: 'PATCH',
    //     headers: { 'Content-Type': 'application/json' },
    //     body: JSON.stringify({ verificationStatus: targetStatus })
    //   });

    const response = await updateTicketStatus(ticketId, { verificationStatus: targetStatus });

      if (response.modifiedCount > 0) {
        // Instant state updates array filter logic
  ...
  return (
    <div className="space-y-4">
      {/* Card wrapper layout mapping system */}
      <div className="bg-white dark:bg-zinc-900 rounded-3xl shadow-xl shadow-zinc-200/40 dark:shadow-none border border-zinc-100 dark:border-zinc-800/80 overflow-hidden">
          ...
          <table className="w-full border-collapse text-left">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-950/40 border-b border-zinc-100 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 text-xs font-bold uppercase tracking-wider">
                ...
                <th className="p-4">Network State</th>
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60 text-sm font-semibold text-zinc-700 dark:text-zinc-300">
              {currentItems.map((ticket) => (
                <tr key={ticket._id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-950/20 transition-colors">
                  ...
                  <td className="p-4 font-mono text-xs text-zinc-400">{ticket.vendorEmail}</td>
                  <td className="p-4 uppercase tracking-wider text-xs font-black text-[#1E3A8A] dark:text-blue-400">{ticket.transportType}</td>
                  <td className="p-4 font-black text-zinc-900 dark:text-white">৳ {ticket.pricePerUnit}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 pr-6 text-right space-x-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center p-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20 disabled:opacity-30 transition-all"
                      title="Approve Asset"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center p-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 disabled:opacity-30 transition-all"
                      title="Reject Asset"
                    >
        ...
        {/* Mobile View Card Matrix Layout (Clean UI response alternative) */}
            ...
            <div key={ticket._id} className="p-5 space-y-4 bg-white dark:bg-zinc-900 text-left">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs border-y border-zinc-50 dark:border-zinc-800/60 py-2.5">
                ...
                <div>
                  ...
                  <span className="font-black text-zinc-900 dark:text-white">৳ {ticket.pricePerUnit}</span>
                </div>
              </div>
              <div className="flex items-center justify-between pt-1">
                ...
                <div className="space-x-2">
                  <button
                    onClick={() => openActionModal(ticket._id, 'approved', ticket.title)}
                    disabled={ticket.verificationStatus === 'approved'}
                    className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 text-white disabled:opacity-40"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => openActionModal(ticket._id, 'rejected', ticket.title)}
                    disabled={ticket.verificationStatus === 'rejected'}
                    className="px-3 py-1.5 rounded-lg text-xs font-bold bg-rose-600 text-white disabled:opacity-40"
                  >
                    Reject
                  </button>
                </div>
      ...
      {/* Pure Tailwind CSS Client Pagination UI Control Node */}
      {totalPages > 1 && (
      ...
      {/* Pure HTML/Tailwind Action Execution Handshake Confirmation Modal */}
      {modalContext.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in text-left">
          <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-2xl p-6 space-y-6">
            ...
            <div className="flex items-center justify-end space-x-3 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-zinc-100 hover:bg-zinc-200 text-zinc-700 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 transition-colors"
              >
                Abort
              </button>
              <button
                onClick={handleStatusUpdateProcess}
                className={`px-5 py-2 rounded-xl text-xs font-black uppercase tracking-wider text-white shadow-lg transition-all active:scale-95 ${
