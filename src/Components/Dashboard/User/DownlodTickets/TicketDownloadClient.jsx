...
// 🎫 ২. পিডিএফে টিকেটের লেআউট কম্পোনেন্ট
...
export default function TicketDownloadClient({ initialTickets = [] }) {
  ...
  if (!initialTickets || initialTickets.length === 0) {
    return (
      <div className="bg-white border rounded-2xl p-12 text-center text-slate-400 font-medium shadow-sm">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {initialTickets.map((ticket) => {
        ...
        return (
          <div key={ticket._id} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6 flex flex-col justify-between">

            {/* স্ক্রিন ভিউ */}
            <div className="bg-white p-5 rounded-2xl border border-dashed border-slate-300 relative space-y-4">
              <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                <div className="flex items-center space-x-2.5">
                  <div className="p-2 bg-blue-50 text-blue-600 rounded-xl"><Ticket className="h-5 w-5" /></div>
                  <div>
                    <h3 className="font-black text-slate-800 text-base">{ticket.ticketTitle}</h3>
                    <p className="text-[10px] text-slate-400 font-mono">ID: #{ticket._id ? ticket._id.slice(-8).toUpperCase() : 'UNKNOWN'}</p>
                  </div>
                </div>
                <span className="px-2.5 py-1 bg-green-100 text-green-700 text-[10px] font-black uppercase rounded-full tracking-wider">
                  {ticket.status || 'PAID'}
                </span>
              </div>

              <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-xl text-xs font-bold text-slate-700">
                <div>
                  <span className="text-[9px] text-slate-400 uppercase block mb-0.5">From</span>
                  <div className="flex items-center text-slate-800"><MapPin className="h-3.5 w-3.5 text-blue-600 mr-1 shrink-0" /> {ticket.from || 'N/A'}</div>
                </div>
                <div>
                  <span className="text-[9px] text-slate-400 uppercase block mb-0.5">To</span>
                  <div className="flex items-center text-slate-800"><MapPin className="h-3.5 w-3.5 text-orange-600 mr-1 shrink-0" /> {ticket.to || 'N/A'}</div>
                </div>
              </div>

              <div className="grid grid-cols-3 gap-2 text-[11px] font-semibold text-slate-600">
                <div>
                  <span className="text-[9px] text-slate-400 uppercase block mb-0.5">Date</span>
                  <div className="flex items-center font-bold text-slate-800"><Calendar className="h-3.5 w-3.5 text-slate-400 mr-1" /> {formattedDate}</div>
                </div>
                <div>
                  <span className="text-[9px] text-slate-400 uppercase block mb-0.5">Time</span>
                  <div className="flex items-center font-bold text-slate-800"><Clock className="h-3.5 w-3.5 text-slate-400 mr-1" /> {formattedTime}</div>
                </div>
                <div className="text-right">
                  <span className="text-[9px] text-slate-400 uppercase block mb-0.5">Seats</span>
                  <span className="font-mono font-black text-slate-800 text-sm">{ticket.bookingQuantity || 0} Pcs</span>
                </div>
              </div>

              <div className="border-t border-slate-100 pt-3 flex justify-between items-center">
                <div>
                  <span className="text-[9px] text-slate-400 uppercase block">Passenger</span>
                  <span className="text-xs font-bold text-slate-700">{ticket.userName}</span>
                </div>
                <div className="text-right">
                  <span className="text-[9px] text-slate-400 uppercase block">Total Price</span>
                  <span className="text-base font-black text-blue-700">৳{(ticket.totalPrice || 0).toLocaleString()}</span>
                </div>
              </div>
            </div>

            {/* 📥 নতুন বাটন যা কাস্টম ফাংশন কল করে টোস্ট দেখাবে */}
            <button
              onClick={() => handleDownload(ticket)}
              disabled={downloadingId === ticket._id}
              className="w-full flex items-center justify-center py-3 bg-[#1E3A8A] hover:bg-[#122554] text-white text-xs font-bold rounded-xl transition-all shadow-md disabled:opacity-60"
            >
